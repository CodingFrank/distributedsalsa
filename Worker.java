...
public class Worker extends UniversalActor  {
	...
	public Worker(UAN __uan, UAL __ual, Object obj) {
		...
		if (obj instanceof UniversalActor.State || obj==null) {
			  ...
			  if (__ual != null && !__ual.getLocation().equals(ServiceFactory.getTheater().getLocation())) {
			    ...
			    createRemotely(__uan, __ual, "assignment2.Worker", sourceRef);
			  }
	...
	public class State extends UniversalActor .State {
		...
		public State(UAN __uan, UAL __ual) {
			super(__uan, __ual);
			addClassName( "assignment2.Worker$State" );
		...
		public MatrixSaver[] calculateDistances(int startStar, int endStar, Double[][] data) {
			long startTime = System.currentTimeMillis();
			{
				// standardOutput<-println("Current thread is computing distances from star "+startStar+" through "+endStar)
				{
					Object _arguments[] = { "Current thread is computing distances from star "+startStar+" through "+endStar };
					Message message = new Message( self, standardOutput, "println", _arguments, null, null );
					__messages.add( message );
				}
			}
			MatrixSaver[] results = new MatrixSaver[data.length-startStar];
			for (int i = 0; i<results.length; i++)results[i] = new MatrixSaver(startStar+i);
			for (int star = startStar; star<=endStar; star++){
				for (int i = star+1; i<data.length; i++){
					double dist = Math.sqrt(Math.pow(data[star][0]-data[i][0], 2)+Math.pow(data[star][1]-data[i][1], 2)+Math.pow(data[star][2]-data[i][2], 2));
			...
			{
				// standardOutput<-println("Current thread has done the computing from stars "+startStar+" through "+endStar+" with an execution time of : "+(endTime-startTime)/1000+"seconds.")
				{
					Object _arguments[] = { "Current thread has done the computing from stars "+startStar+" through "+endStar+" with an execution time of : "+(endTime-startTime)/1000+"seconds." };
					Message message = new Message( self, standardOutput, "println", _arguments, null, null );
					__messages.add( message );
				}
			}
